...
const AbsiArtikelBap = ({ navigation }) => {
  const [dataArtikel] = useState([
    { id: '1', kodeArtikel: 'F', namaArtikel: 'Hicoop Boxer Karet Luar Mix Max Seri 2-1 (L)' },
    { id: '2', kodeArtikel: 'G', namaArtikel: 'Hicoop Boxer Karet Luar Mix Max Seri 2-1 (L)' },
    { id: '3', kodeArtikel: 'H', namaArtikel: 'Hicoop Boxer Karet Luar Mix Max Seri 2-1 (L)' },
    { id: '4', kodeArtikel: 'A', namaArtikel: 'Hicoop Boxer Karet Luar Mix Max Seri 2-1 (L)' },
    { id: '5', kodeArtikel: 'B', namaArtikel: 'Hicoop Boxer Karet Luar Mix Max Seri 2-1 (L)' },
  ]);
  ...
  const handleNavigate = () => {
    if (selectedCount === 0) {
      ...
      const updatedItems = selectedItems.map((item, index) => ({
        ...item,
        id: (dataArtikel.length + index + 1).toString(),
        quantity: (parseInt(item.quantity) + dataArtikel.length + index + 1).toString()
      }));
      navigation.navigate('KeranjangBap', { selectedItems: updatedItems });
    }
  };
